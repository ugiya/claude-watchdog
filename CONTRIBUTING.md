...
exact agent application and version when available, OS and version, Python
...
and installer validations, including detached execution of the installed ZIP. On macOS or Linux, also run the isolated lifecycle and PTY
dashboard checks:
...
never publish raw captures. The integration command requires macOS or Linux. uv downloads the requested interpreter
...
automated tests must never invoke a real `pmset sleepnow` or `systemctl suspend`.
...
- `tests/integration/`: isolated per-platform lifecycle and terminal runners; not part
...
macOS and Linux contributors should also run `python3 scripts/check.py --integration`.
